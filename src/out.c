...
enum {
#define ELT(enum_symbol, _) enum_symbol,
  INSTR_OP_ARRAY(ELT)
#undef ELT
};

static inline const char *instr_op_mneumonic(int op)
{
  static const char *arr[] = {
    #define ELT(_, str) str,
    INSTR_OP_ARRAY(ELT)
    #undef ELT
  };
...
static instr_fmt_t instr_tbl[] = {
  ...
  {  OP_CALLF,     0xff,      3,   OPER_M32,     -1,           -1            },
  {  OP_JMP,       0xff,      4,   OPER_RM16,    -1,           -1            },
  {  OP_JMPF,      0xff,      5,   OPER_M32,     -1,           -1            },
