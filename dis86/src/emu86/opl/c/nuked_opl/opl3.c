...
// NOTE(AJB): Sine-wave cycle is mapped [0, 2pi] => [0, 0x400]
// But we exploit symmetry of the function and only build a table for the
// [0, pi/2] (or [0, 0xff]) range. It's easy to construct the full-wave
// or modifications (e.g. rectified). As log(sin(x)) is negative on [0, pi/2]
// we actually compute -log(sin(x)). Finally, the output is encoded as a fixed-point
// format with the lower 8-bits being fractional bits
//
// The formula for this calculation appears to be:
//   logsinrom[i] = round(256 * -log(sin(pi/2 * (i+0.5)/256)))
//
...
// NOTE(AJB) Not entirely sure of the formula here. The following
// is quite close but not exact:
//   exprom[i] = round(0x7fa * 2**(-x/256))
//
...
{
    // NOTE(AJB): Level is an implied negative fixed-point number with lower 8-bits as fractional
    // The LUT is used only for the fractional bits since the whole bits are easy to compute
    // That is: x * 2**(-n) = x >> n
    if (level > 0x1fff)
    {
        level = 0x1fff;
    }
    // NOTE(AJB) Do lookup with lower 8-bits, scale up by 2 (assuming this was done to store one
    // less bit per table entry ... output range appears to be [-0x1000, 0x1000]).
    // Then apply the whole number part via ">> (level >> 8)"
...
static int16_t OPL3_EnvelopeCalcSin0(uint16_t phase, uint16_t envelope)
{
    uint16_t out = 0;
    uint16_t neg = 0;
    // NOTE(AJB): MODULUS the phase to [0, 0x400). Note there doesn't seem to be any other
    // truncation method, it just overflows as 2^16, which I beleive is safe
    phase &= 0x3ff;
    // NOTE(AJB): The range [0x200, 0x400) is the negative range.. it's a y-axis reflection of the
    // range [0, 0x100)
    if (phase & 0x200)
    {
        neg = 0xffff;
    }
    // NOTE(AJB): The range [0x100, 0x200) is the same as [0, 0x100) refelcted through a
    // vertical line at x=0x100
    if (phase & 0x100)
    {
        // NOTE(AJB) Naively this reflection changes our lookup index as:
        //  index_new = 0xff - (index_old - 0x100)
        //            = 0x1ff - index_old                      [simplify]
        //            = (0x1ff - index_old) & 0xff             [working in 8-bits]
        //            = (0x1ff + ~index_old + 1) & 0xff        [twos-compliment]
        //            = (0x200 + ~index_old) & 0xff            [simplify]
        //            = (0x200 & 0xff) + (~index_old & 0xff)   [distribute]
        //            = ~index_old & 0xff                      [simplify]
        //            = (index_old & 0xff) ^ 0xff              [exchange order of operations]
        // Which gives the implementation below:
        out = logsinrom[(phase & 0xffu) ^ 0xffu];
    }
    else
    {
      // NOTE(AJB): Range [0, 0x100)
        out = logsinrom[phase & 0xffu];
    }
    // NOTE(AJB): Add in the envelope amplitude scaling before computing the exp2()
    // We scale it up by 2**3 = 8, so it probably has 5-bits fractional.
    // Finally, we apply the negation for the [0x200, 0x400) range. This isn't a pure
    // negation in the twos-compliment sense, but the inacuracy of the missing "+1" is viewed
    // as negligible in the [-0x1000, 0x1000] range
...
opl3_chip * OPL3_New(void)
{
  return (opl3_chip*)calloc(1, sizeof(opl3_chip));
}

void OPL3_Delete(opl3_chip *chip)
{
  free((void*)chip);
}
